...
const showWarning = (batchFileListLocation: string, sendMail: boolean) => {
    if (sendMail) {
...
const sendMailIndividualHandler = async (CONFIG: CertificateConfig, eventData: Event[], commonData: Mail.Attachment[]) => {
...
async function csvParserSendIndividual(CONFIG: CertificateConfig) {
    ...
    await sendMailIndividualHandler(CONFIG, eventData, commonData);
}

async function sendCertificatedMail(CONFIG: CertificateConfig) {
    await MailTokenVerified;
    if (!showWarning(CONFIG.dataPath, CONFIG.sendMail)) return;
    await csvParserSendIndividual(CONFIG);
    if (CONFIG.sendMail || CONFIG.sendDevMail) console.log('Email Sending Done'.magenta.bold);
    process.exit(0);
}

export default sendCertificatedMail;
